    ...
    (function () {
        ...
        window.onload = () => {
            ...
            if (normalizedHost.includes('lichess')) {
                site = 'lichess';
            } else if (normalizedHost.includes('chess.com')) {
                site = 'chesscom';
        ...
        function getSelectedMoveRecord() {
            ...
            if (site === 'chesscom') {
                selectedMove = document.querySelector('.node .selected') // vs player + computer (new)
                    ...
                    || document.querySelector('.move-node.selected') // analysis fallback
                    || document.querySelector('.selected'); // generic fallback
            } else if (site === 'lichess') {
        ...
        function getBoard() {
            let board;
            if (site === 'chesscom') {
                board = document.querySelector('.board') || document.querySelector('#board-layout-chessboard') || document.querySelector('wc-chess-board');
            } else if (site === 'lichess') {
                board = document.querySelector('.main-board') || document.querySelector('cg-board');
            } else if (site === 'blitztactics') {
        ...
        function determineStartPosition() {
            ...
            // Initial polling to find the board
            let retryCount = 0;
        ...
        const SCRAPE_THROTTLE = 1000; // 1 second throttle for Safari stability

        function startObservation() {
            let targetNode = null;
            if (site === 'chesscom') {
                // Try to find the most specific container to avoid observing the whole body
                targetNode = document.querySelector('.board-layout-main') ||
                    document.querySelector('wc-chess-board') ||
                    document.querySelector('.board');
            } else if (site === 'lichess') {
                targetNode = document.querySelector('.main-board') || document.querySelector('cg-board');
            }
            ...
            const callback = (mutationList) => {
                ...
                // Use requestAnimationFrame to defer scraping until after the current frame
